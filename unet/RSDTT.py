...
class Channel_Embeddings(nn.Module):
    ...
    def forward(self, x):
        ...
        # 动态计算 n_patches
        n_patches = (H // patch_size_h) * (W // patch_size_w)

        # 如果位置编码未初始化或尺寸不匹配，重新初始化
        if (self.position_embeddings is None or
                self.position_embeddings.shape[1] != n_patches):
            self.position_embeddings = nn.Parameter(
                torch.zeros(1, n_patches, C)
            ).to(x.device)

        # Patch Embedding
        x = self.conv(x)  # (B, C, H//pH, W//pW)
        x = x.flatten(2)  # (B, C, n_patches)
        x = x.transpose(-1, -2)  # (B, n_patches, C)
...
class Block_ViT(nn.Module):
    def __init__(self, config, vis, channel_num):
        ...
        self.attn_norm1 = DyT(channel_num[0])
        self.attn_norm2 = DyT(channel_num[1])
        self.attn_norm3 = DyT(channel_num[2])
        self.attn_norm4 = DyT(channel_num[3])
        self.attn_norm = DyT(config.KV_size)

        self.channel_attn = Attention_org(config, vis, channel_num)


        self.ffn_norm1 = DyT(channel_num[0])
        self.ffn_norm2 = DyT(channel_num[1])
        self.ffn_norm3 = DyT(channel_num[2])
        self.ffn_norm4 = DyT(channel_num[3])
...
class Encoder(nn.Module):
    def __init__(self, config, vis, channel_num):
        ...
        self.encoder_norm1 = DyT(channel_num[0])
        self.encoder_norm2 = DyT(channel_num[1])
        self.encoder_norm3 = DyT(channel_num[2])
        self.encoder_norm4 = DyT(channel_num[3])
